            ...
            if self._should_create_picking_real_time():
                ...
                for location_id_from_popup in locations_from_popup:
                    ...
                    picking = self.env['stock.picking']._create_picking_from_pos_order_lines(
                        destination_id, lines_with_location, picking_type, self.partner_id, location_id, pos_session_id
                    )
                ...
                if lines_without_location:
                    ...
                    if self.partner_id.property_stock_customer:
                        ...
                        destination_id = picking_type.default_location_dest_id.id
                    picking = self.env['stock.picking']._create_picking_from_pos_order_lines(
                        destination_id, lines_without_location, picking_type, self.partner_id, pos_session_id
                    )
...
class PosOrderline(models.Model):
    ...
    # make sure a reference to the  is available
        ...
        # Log the POS session ID
        # pos_session_id = self.env.context.get('pos_session_id')
        # _logger.info("The value of _create_picking_from_pos_order_lines move.reference = %s, POS session ID = %s",
        #              pos_session_id.name if pos_session_id else 'N/A', pos_session_id.id if pos_session_id else 'N/A')
...
class StockPicking(models.Model):
    ...
    @api.model
    def _create_picking_from_pos_order_lines(self, location_dest_id, lines, picking_type, partner=False, location_id=None, pos_session_id=None):
        pickings = self.env['stock.picking']
        ...
        if pos_session_id:
            _logger.info("POS Session ID: %s", pos_session_id.id)
            _logger.info("POS Session Name: %s", pos_session_id.name)
            # Access other fields of the pos_session_id
            _logger.info("POS Session Start Date: %s", pos_session_id.start_at)
            _logger.info("POS Session User: %s", pos_session_id.user_id.name)
        ...
        if positive_lines:
            ...
            positive_picking.pos_session_id = pos_session_id
        ...
        if negative_lines:
            ...
            negative_picking.pos_session_id = pos_session_id
    ...
    #     for line in lines:
    #         move_vals = self._prepare_stock_move_vals(line)
    #         move = self.env['stock.move'].create(move_vals)
    #         move.reference = '{}/{}/{}'.format(
    #             move.picking_id.picking_type_id.warehouse_id.code,
    #             'POS',
    #             move.picking_id.pos_session_id.id
    #         )
    #         moves |= move
    #     return moves
...
class StockMove(models.Model):
    ...
    reference = fields.Char(
        'Reference', compute='_compute_reference', store=True)

    # @api.model
    # def create(self, vals):
    #     # Custom logic to set reference
    #     vals['reference'] = 'Custom Reference'  # Adjust as needed
    #     return super(StockMove, self).create(vals)

    @api.depends('picking_id', 'name')
    def _compute_reference(self):
        for move in self:
            if move.picking_id.pos_session_id:
                # Get the warehouse code and POS session ID
                warehouse_code = move.picking_id.picking_type_id.warehouse_id.code or 'wh'
                # Ensure pos_session_id is properly set
                pos_session_id = move.picking_id.pos_session_id.name or ''
                # move.reference = f'{warehouse_code}/POS/{pos_session_id}'
                move.reference = f'{warehouse_code}/{pos_session_id}'
                print("the value of _compute_reference move.reference = ",
                      move.reference, warehouse_code, pos_session_id)
                print("the value of _compute_reference pos_session_id =",
                      pos_session_id)

            else:
                # Fallback if picking_id is not set
                move.reference = move.picking_id.name if move.picking_id else move.name
            print(
                "the value of _compute_reference POS session ID move.reference = ", move.reference)
